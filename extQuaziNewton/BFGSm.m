...
% BFGS('Objectivefunction_',[0.5 0.5],20, 0.0001, 0,1 ,20)
%
function ReturnValue = BFGSm(functname, ...
      dvar0,niter,tol,lowbound,intvl,ntrials, nwzfp)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if 0 % (nvar == 2)
   ...
   %  previous generation code is left in place
    ...
    for i = 1:x1len;
        for j = 1:x2len;
            x1x2 =[x1(i) x2(j)];
            fun(j,i) = feval(functname,x1x2,nwzfp.Test,nwzfp.LTE_sb_2_up2,nwzfp.LTE_sb_5_up2,nwzfp.grid,nwzfp.ant,nwzfp.prec,nwzfp.far,nwzfp.dl2,nwzfp.ul,nwzfp.av,nwzfp.ah,nwzfp.offset);
        end
    end
...
x = dvar0;
Lc = 'r';
fs(1) = feval(functname,x,nwzfp.Test,nwzfp.LTE_sb_2_up2,nwzfp.LTE_sb_5_up2,nwzfp.grid,nwzfp.ant,nwzfp.prec,nwzfp.far,nwzfp.dl2,nwzfp.ul,nwzfp.av,nwzfp.ah,nwzfp.offset); % value of function at start
as(1)=0;
grad = (gradfunctionm(functname,x,nwzfp)); % steepest descent
...
for i = 1:niter-1
    ...
    output = GoldSection_nVarm(functname,tol,x, ...
        s,lowbound,intvl,ntrials,nwzfp);
    ...
    delx = (x - xs(i,:))';
    gradold = gradfunctionm(functname,xs(i,:),nwzfp);
